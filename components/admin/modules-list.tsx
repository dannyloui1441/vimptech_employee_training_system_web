...
function DayCard({
  ...
  const [editData, setEditData] = useState({
    ...
    gapValue: module.gapValue ?? 0,
  })

  const handleSaveEdit = () => {
    onEdit(module.id, { module: editData.module, gapValue: editData.gapValue, gapUnit: 'days' })
    setIsEditing(false)
  }
      ...
      {isEditing && (
        <div className="px-4 py-3 border-t border-b bg-muted/10 space-y-3">
          <div className="flex items-end gap-3 flex-wrap">
            <div>
              <Label className="text-xs">Module Order</Label>
              <Input
                type="number"
                min="1"
              ...
              />
              <p className="text-xs text-muted-foreground mt-0.5">Sequence position</p>
            </div>
            {subjectMode === "scheduled" && (
              <div>
                <Label className="text-xs">Days Until Unlock</Label>
                <Input
                  type="number"
                  min="0"
                  ...
                  className="h-8 w-24 text-sm"
                />
                <p className="text-xs text-muted-foreground mt-0.5">Days after previous unlock</p>
              </div>
            )}
            <div className="flex gap-2">
