...
function mapModuleProgress(row: any): ModuleProgress {
  return {
    id: row.id,
    userId: row.user_id,
    subjectId: row.subject_id,
    moduleId: row.module_id,
    contentProgressPercent: row.content_progress_percent ?? 0,
    assessmentPassed: row.assessment_passed ?? false,
...
export const db = {
  ...
  moduleProgress: {
    /**
     * Safe upsert for module_progress.
     *
     * Uses explicit SELECT → UPDATE or INSERT (no Supabase .upsert()).
     * Only writes fields that are explicitly provided, so:
     *   - assessment route won't reset content_progress_percent
     *   - progress route won't reset assessment_passed
     */
    async upsert(data: {
      userId: string;
      subjectId: string;
      ...
      // 2. Row exists → partial UPDATE (only provided fields)
      if (existing) {
        const updatePayload: any = { updated_at: now };

        if (data.contentProgressPercent !== undefined) {
          updatePayload.content_progress_percent = data.contentProgressPercent;
        }
        if (data.assessmentPassed !== undefined) {
          updatePayload.assessment_passed = data.assessmentPassed;
        }
        if (data.contentCompletedAt !== undefined) {
          updatePayload.content_completed_at = data.contentCompletedAt;
        }
        ...
        if (error) throw error;
        return mapModuleProgress(updated);
      }

      // 3. Row does not exist → INSERT with safe defaults
      const insertPayload = {
        id: `mp-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
        user_id: data.userId,
        subject_id: data.subjectId,
        module_id: data.moduleId,
        content_progress_percent: data.contentProgressPercent ?? 0,
      ...
      console.log('[moduleProgress.upsert] INSERT new row', {
        userId: data.userId,
        moduleId: data.moduleId,
        insertPayload,
      });
      ...
      if (error) throw error;
      return mapModuleProgress(inserted);
    },

    async findByUserAndModule(userId: string, moduleId: string): Promise<ModuleProgress | null> {
      const { data, error } = await supabase
        .from('module_progress')
        .select('*')
        .eq('user_id', userId)
        ...
        .single();
      if (error) return null;
      return mapModuleProgress(data);
    },

    async findByUser(userId: string): Promise<ModuleProgress[]> {
      const { data, error } = await supabase
        .from('module_progress')
        .select('*')
        .eq('user_id', userId);
